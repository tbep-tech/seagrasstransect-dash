...
# base map
m <- tbeptools::util_map(trnpts, minimap = NULL) %>% 
...
names(cols) <- sort(unique(trnlns$MonAgency))

tbeptools::util_map(trnpts, minimap = NULL) %>% 
  leaflet::addPolylines(
    data = trnlns,
    color = as.character(cols[as.character(trnlns$MonAgency)]),
    weight = 8,
    opacity = 0.9,
    label = ~paste0('Transect ', Site, ' (Agency responsible: ', MonAgency, ')')
  ) %>% 
