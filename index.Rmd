...
trns <- sort(intersect(unique(transect$Transect), trnpts$TRAN_ID))
...
aveplo <- reactive({
  ...
  p <- show_transectavespp(transectocc, bay_segment = segsel1, species = sppsel1, plotly = T, family = fml, yrrng = yrssel1, total = tots, sppcol = sppcol) %>% 
    layout(
      title = list(text = NA)
    )
...
trnplo <- reactive({
  ...
  p <- show_transect(transect, site = trnsel1, species = sppsel2, varplo = 'Abundance', plotly = T, base_size = 11, yrrng = yrssel2, sppcol = sppcol) %>% 
    layout(
      title = list(text = NA), 
...
sumplo <- reactive({
  ...
  p <- show_transectsum(transectocc, site = trnsel1, species = sppsel2, yrrng = yrssel2, sppcol = sppcol) %>% 
    layout(
      title = list(text = NA),
...
fillCol(flex = c(0.1, 1),
  fillRow(
    ...
    sliderInput(inputId = 'yrssel1', label = 'Select year range:', min = 1998, max = 2021, value = c(1998, 2021), step = 1, sep = '', width = '90%')
  ),
...
fillCol(flex = c(0.1, 1),
  fillRow(
    ...
    sliderInput(inputId = 'yrssel2', label = 'Select year range:', min = 1998, max = 2021, value = c(1998, 2021), step = 1, sep = '', width = '90%')
  ),
  fillCol(flex = c(1, 1),
    renderPlotly(trnplo()),
    renderPlotly(sumplo())
