...
matplo <- reactive({
  ...
  p <- show_transectmatrix(transectocc, bay_segment = segsel1, plotly = T, family = fml, neutral = T, yrrng = yrssel1, total = tots)
...
demplo <- reactive({
  ...
  p <- ggplot(demdat, aes(y = Depth_dem, x = Site, group = Date)) + 
    geom_line() + 
    geom_point(aes(colour = patxt, size = meanabu)) + 
    facet_wrap(~Date) +
    scale_size('Mean abundance', range = c(2, 6)) + 
...
trnptstab <- reactive({
  ...
  out <- trnpts %>% 
    st_set_geometry(NULL) %>% 
    select(Transect = TRAN_ID, lat_dd = LAT_DD, lon_dd = LONG_DD, mon_agency = MonAgency, bay_segment)
...
1) __OVERALL SUMMARIES__: View stoplight graphic and changes over time by major bay segments
1) __INDIVIDUAL TRANSECTS__: View results for individual transects
1) __SEAGRASS EDGE__: View results for estimates of the maximum seagrass edge for individual transects (experimental)
1) __DATA DOWNLOADS__: Download transect data and metadata
...
Technical questions about the seagrass transect data can be sent to [Gary Raulerson](mailto:graulserson@tbep.org).  Questions and comments about the dashboard can be sent to [Marcus Beck](mailto:mbeck@tbep.org).
...
The __report card__ provides an assessment of relative health of seagrasses in each of the major bay segments, including a baywide assessment in the Tampa Bay column.  The categories (red, yellow, orange, green) are based on average frequency occurrence estimates across all species in each segment, with breakpoints for the categories at 0%, 25%, 50%, 75%, and 100% frequency occurrences.  Green means higher frequency occurrences (healthy seagrass) and red means lower frequency occurrences (less healthy seagrass).  
...
Similarly, species can be added or removed from the right plot by changing the selection. Below, Halodule is selected and ready to be removed by hitting the backspace key on your keyboard.
...
fillCol(flex = c(0.1, 1),
  fillRow(
    ...
    selectInput(inputId = 'sppsel1', label = 'Select species:', choices = c('total', spp), selected = c('total', spp[1:5]), selectize = T, multiple = T, width = '90%'), 
    sliderInput(inputId = 'yrssel1', label = 'Select year range:', min = 1998, max = 2020, value = c(1998, 2020), step = 1, sep = '', width = '90%')
  ),
...
Species can be added or removed from both plots by changing the selection in the top middle selector. Below, Halodule is selected and ready to be removed by hitting the backspace key.
...
fillCol(flex = c(0.1, 1),
  fillRow(
    ...
    selectInput(inputId = 'sppsel2', label = 'Select species:', choices = spp, selected = spp[1:5], selectize = T, multiple = T, width = '90%'), 
    sliderInput(inputId = 'yrssel2', label = 'Select year range:', min = 1998, max = 2020, value = c(1998, 2020), step = 1, sep = '', width = '90%')
  ),
  fillCol(flex = c(1, 1),
    renderPlotly(trnplo()),
    renderPlotly(sumplo())
...
```{r}
fillCol(flex = c(NA, 1),
  downloadBttn('dltrnptstab', 'Download data', style = 'simple', block = T, color = 'success'),
  renderReactable(rcttrnptstab())
  )
```
